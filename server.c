...
int main(){
	...
	if(setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &optval, optlen)==-1){
...
void service_client(int sock_fd){
	...
	while(1){		
		if(func_id==1){login(sock_fd);read(sock_fd, &func_id, sizeof(int));}
		if(func_id==2) {signup(sock_fd);read(sock_fd, &func_id, sizeof(int));}
		if(func_id==3) break;
	}
	close(sock_fd);
...
int login(int sock_fd){
	...
	if(type == 1){
		// customer
		lock.l_type = F_WRLCK;
		fcntl(fd,F_SETLKW, &lock);// wait if not available ******
		...
		// Admin
		lock.l_type = F_WRLCK;
		int ret = fcntl(fd,F_SETLK, &lock); // changed F_SETLKW
		if(ret != -1){
...
int admin_menu(int sock_fd, int id){
	...
	if(op_id == 1){
		...
		temp.tid = tid;
		temp.train_no = tno; //input train no. from socket
		...
		lock.l_type = F_WRLCK;
		lock.l_start = 0;
		lock.l_len = 0; // complete file lock
		lock.l_whence = SEEK_SET;
		lock.l_pid = getpid();
	...
	if(op_id == 2){
		...
		read(sock_fd, &no_of_trains, sizeof(int)); //here no_of_trains is interpreted as train id
		if(no_of_trains != -2)			//returns true when admin wants to delete a train
		{
			...
			read(fd, &temp, sizeof(struct train));			
			printf("%s is deleted\n", temp.train_name);
			strcpy(temp.train_name,"deleted");   //replacing train name with "deleted"
			lseek(fd, -1*sizeof(struct train), SEEK_CUR);
	...
	if(op_id == 3){
		...
		write(sock_fd, &no_of_trains, sizeof(int));
		lseek(fd, 0, SEEK_SET);
		// printing all available train details in the system.
		while(fp != lseek(fd, 0, SEEK_CUR)){
			struct train temp;
			read(fd, &temp, sizeof(struct train));
			write(sock_fd, &temp.tid, sizeof(int));
...
int user_menu(int sock_fd, int id, int type){
	...
	if(op_id == 3){
		...
		read(sock_fd, &val, sizeof(int));	//Read the Booking ID to updated
		...
		read(sock_fd, &val, sizeof(int));	//Increase or Decrease
	...
	if(op_id == 4) {
		...
		struct train temp1;
		struct bookings temp2;
		read(sock_fd, &val, sizeof(int));	//Read the Booking ID to deleted
		//lseek(fd2, 0, SEEK_SET);
...
void view_booking(int sock_fd, int id, int type){
	...
	if(fp == 0)
		...
		struct bookings bk[10];// maximum no. of booking
